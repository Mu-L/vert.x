...
The simplest way to create an HTTP server, using all default config is as follows:
...
If you don't want the default, a server can be configured by passing in a {@link io.vertx.core.http.HttpServerConfig}
...
With plain text (TLS is disabled), the server handle `h2c` requests and will upgrade any request HTTP/1.1 that wants to
upgrade to HTTP/2. It will also accept a direct `h2c` (with prior knowledge) connection beginning
with the `PRI * HTTP/2.0\r\nSM\r\n` preface.

WARNING: browsers do not support `h2c`, for serving websites you should use `h2` and not `h2c`.

When a server accepts an HTTP/2 connection, it sends to the client its {@link io.vertx.core.http.Http2ServerConfig#getInitialSettings initial settings}.
...
By default, an HTTP server accepts HTTP/1.1 and HTTP/2 protocols.

Supported HTTP versions depends on {@link io.vertx.core.http.HttpServerConfig} versions.

[source,$lang]
----
{@link examples.HTTPExamples#setServerVersions}
----
...
Use {@link io.vertx.core.http.HttpServerRequest#query} to return the query string part of the URI
...
For HTTP/1.x requests the `host` header is returned, for HTTP/2 requests the `:authority` pseudo header is returned.
...
To enable decompression set {@link io.vertx.core.http.HttpCompressionConfig#setDecompressionEnabled(boolean)} on the
...
closed after an idle time, then you configure {@link io.vertx.core.http.HttpServerConfig#setIdleTimeout}.
...
To enable compression use can configure it with {@link io.vertx.core.http.HttpCompressionConfig#setCompressionEnabled}.
...
The minimum response content size threshold for compression can be configured via {@link io.vertx.core.http.HttpCompressionConfig#setContentSizeThreshold}.
...
Vert.x supports `h2` (HTTP/2 over TCP/TLS) and `h2c` (HTTP/2 over TCP).

=== Making SSL requests

By default, the client supports HTTP/1.1 and HTTP/2

[source,$lang]
----
{@link examples.HTTP2Examples#example7_1}
----

When making a requests, the client SSL handshake will sends both `http/1.1` and `h2` protocols and the server decides
which protocol to use.

=== Making plain text requests

By default, the http client performs HTTP/1.1 requests.

To perform HTTP/2 requests the {@link io.vertx.core.http.HttpClientConfig#setVersions} must be set to `(HTTP/2, HTTP/1.1)`.

[source,$lang]
----
{@link examples.HTTP2Examples#example7_2}
----

The client will do an HTTP/1.1 requests and try an upgrade to HTTP/2.

`h2c` connections can also be established directly, i.e. with connection started with a prior knowledge, when
{@link io.vertx.core.http.Http2ClientConfig#setClearTextUpgrade(boolean)} options is set to false: after the
...
The server may not support HTTP/2, the actual version can be checked with {@link io.vertx.core.http.HttpClientResponse#version()} when the response arrives.

When a client connects to an HTTP/2 server, it sends to the server its {@link io.vertx.core.http.Http2ClientConfig#getInitialSettings initial settings}.
...
In addition to {@link io.vertx.core.http.HttpClientConfig} and {@link io.vertx.core.http.PoolOptions}, you
can set
...
You can use {@link io.vertx.core.http.Http2ServerConfig#setRstFloodMaxRstFramePerWindow} and {@link io.vertx.core.http.Http2ServerConfig#setRstFloodWindowDuration} to override these settings.
...
This is done by setting the option {@link io.vertx.core.http.HttpServerConfig#setHandle100ContinueAutomatically(boolean)}.
...
To enable decompression set {@link io.vertx.core.http.HttpClientConfig#setDecompressionEnabled(boolean)} on the options
...
For pooling to work, keep alive must be true using {@link io.vertx.core.http.Http1ClientConfig#setKeepAlive(boolean)}
...
You can set the default timeout using {@link io.vertx.core.http.Http1ClientConfig#setKeepAliveTimeout(java.time.Duration)} - any
...
To enable pipe-lining, it must be enabled using {@link io.vertx.core.http.Http1ClientConfig#setPipelining(boolean)}.
By default, pipe-lining is disabled.

When pipe-lining is enabled requests will be written to connections without waiting for previous responses to return.

The number of pipe-lined requests over a single connection is limited by {@link io.vertx.core.http.Http1ClientConfig#setPipeliningLimit}.
...
pool instead of a single connection, {@link io.vertx.core.http.Http2ClientConfig#setMultiplexingLimit(int)}
can be used.
...
Alternatively you can set idle timeout using {@link io.vertx.core.http.HttpClientConfig#setIdleTimeout(java.time.Duration)} - any
...
are configured by {@link io.vertx.core.http.Http2ClientConfig#setInitialSettings} on the client and
{@link io.vertx.core.http.Http2ServerConfig#setInitialSettings} on the server.
...
The {@link io.vertx.core.http.HttpClientConfig#setSsl(boolean)} setting acts as the default client setting.
...
You can use {@link io.vertx.core.net.TcpClientConfig#setNonProxyHosts} to configure a list of host bypassing the proxy.
